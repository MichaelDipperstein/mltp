...
/*     -pP : P = number of threads.  P must be power of 2.               */
/*     -vV : V = the number of virtual processors.                       */
...

{
   ...
   while ((ch = getopt(argc, argv, "n:p:v:e:r:t:soh")) != -1) {
     switch(ch) {
     ...
     case 'p': nprocs = atoi(optarg);
               ...
               break;
     case 'v': nvps = atoi(optarg);
               if (nvps < 1) {
                 printerr("V must be >= 1\n");
                 exit(-1);
               }
               break;
     ...
     case 'h': printf("Usage: OCEAN <options>\n\n");
               ...
               printf("  -pP : P = number of threads.  P must be power of 2.\n");
               printf("  -vV : V = number of virtual processors.\n");
               ...
               printf("Default: OCEAN -n%1d -p%1d -v%1d -e%1g -r%1g -t%1g\n",
                       DEFAULT_N,DEFAULT_P,DEFAULT_P,DEFAULT_E,DEFAULT_R,
                       DEFAULT_T);
               exit(0);
               break;
     }
...
*/
   for (i=0;i<nprocs;i++) {
     /* mytp version cannot run slave in main process */
     CREATE(slave)  
   }
